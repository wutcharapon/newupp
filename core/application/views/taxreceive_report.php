...
if (!empty($taxreceive_report) || !empty($taxreceive_report_old)) {
    $pdf->AddPage('P', PDF_UNIT, 'Letter');
    $pdf->SetFillColor(255, 255, 255);
    // print_r($paytaxno_report);
    
  if(!empty($taxreceive_report)){
    foreach ($taxreceive_report as $row) {
        $docno = $row['TA_TAX_NO']; 
...
}else if (!empty($taxreceive_report_old)){

    foreach ($taxreceive_report_old as $row) {
        $docno = $row['docno']; 
        ...
        $ddate = $row['ddate'];
        $des = $row['des'];
        $amt = decimalPart($row['amt']);
        $tax = decimalPart($row['tax']);
        $thaiBath = thaiBath($row['tax']);
