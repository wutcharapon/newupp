...
class Custom_upload {
	...
	function __construct($array = array()) {
        ...
        $this->config['allowed_types'] = (array_key_exists('allowed_types', $array)) ? $array['allowed_types'] : 'gif|jpg|jpeg|png';
        $this->config['max_size'] = (array_key_exists('max_size', $array)) ? $array['max_size'] : '3024';
        $this->config['max_width'] = (array_key_exists('max_width', $array)) ? $array['max_width'] : '6000';
        $this->config['max_height'] = (array_key_exists('max_height', $array)) ? $array['max_height'] : '6000';
    ...
    function uploadFilePDF($input, $new_filename = '', $max_size='1024') {
        if ($new_filename)
    ...
    function uploadImage($input, $thumb = false, $new_filename = '', $dimension = false, $canvasW = 150, $canvasH = 150) {
        ...
        if (!$this->obj->upload->do_upload($input)) {
            ...
            if ($dimension) {
                ...
                $data['thumb_name'] = $this->ImageResize($this->config['upload_path'] . $data['orig_name'], $this->config['upload_path'], $canvasW, $canvasH, $thumb_name);
            } else {
                if ($thumb) {
    ...
    function ImageResize($source_image, $path, $canvasW = 150, $canvasH = 150, $set_name = NULL) {
